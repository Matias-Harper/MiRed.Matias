...
def obtener_nombre():
    nombre = input("Para empezar, ¿Como te llamas perrito malvado? ")
    nombre = nombre.capitalize()
    return nombre

def obtener_edad():
    agno = int(input("Para preparar tu perfil, ¿en que año naciste? "))
    return 2022-agno-1

def obtener_estatura():
    estatura = float(input("¿Cuanto medis? damelo en metros plis y usa punto (no la coma)  "))
    metros = int(estatura)
    centimetros = int( (estatura - metros)*100 )
    return (metros, centimetros)

def obtener_sexo():
    sexo = input("Por favor, ingresa tu sexo (M =Masculino, F =Femenino, X = Otro o prefiere no contestar: ")
    sexo = sexo.upper()
...
def opcion_menu():
    ...
    print("  5. Cambiar a otro usuario")
    print("  6. ¿Quieres agregar otro amigo?   ")
    print("  7. Mostar ultimos estados de mis amigos  ")
    print("  0. Salir")
...
def escritura_archivo(nombre, edad, estatura_m, estatura_cm, sexo, pais, amigos, estado, muro):
